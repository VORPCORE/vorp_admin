...
Citizen.CreateThread(function()
    updatePath = "/VORPCORE/vorp_admin" -- your git user/repo path
    resourceName = "FiveM-Resource-Version-Check-Thing (" .. GetCurrentResourceName() .. ")" -- the resource name
    print(updatePath)
    function checkVersion(err, responseText, headers)
        curVersion = LoadResourceFile(GetCurrentResourceName(), "version.json") -- make sure the "version" file actually exists in your resource root!
        print(curVersion) -- works
        print(responseText)
        if curVersion ~= responseText and tonumber(curVersion) < tonumber(responseText) then
            print("\n###############################")
            print("\n" .. resourceName .. " is outdated, should be:\n" .. responseText .. "is:\n" .. curVersion .. "\nplease update it from https://github.com/VORPCORE/vorp_admin")
            print("\n###############################")
